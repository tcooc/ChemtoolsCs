...
namespace ChemTools {
	...
	public class ChemTools {
		...
		[STAThread] //single threaded apartment (stops everything from freaking out)
		...
		public static string CalculateFormula(string input) {
			...
			for(; startIndex < input.Length && (char.IsDigit(input, startIndex) || input[startIndex] == '.'); startIndex++) {
				...
				if(number.Length > 0) {
					if(startIndex < input.Length && input[startIndex] == 'g') {
						startIndex++;
						grams = double.Parse(number);
					} else {
						return "Invalid number format for amount.";
					}
				}
			//end
		...
		public static string CalculateEquation(string input) {
			...
			Equation equation;
			try {
				equation = EquationParser.ParseEquation(input);
			} catch {
